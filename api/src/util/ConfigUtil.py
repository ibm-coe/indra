...
class ConfigUtil :
    ...
    def _loadConfigFile(self, fileName):
        ...
        correct_file_path = r'C:\Users\kundena sridhar\Downloads\integration_hub_v2-feature-webhook-integration (1)\integration_hub_v2-feature-webhook-integration\api\config\envizi-config.json'
        data = {}
        try:
            with open(correct_file_path, "r") as json_file:
                data = json.load(json_file)
                self.logger.debug(data)
        except FileNotFoundError:
            self.logger.error(f"The file '{correct_file_path}' does not exist.")
        except json.JSONDecodeError:
            self.logger.error(f"The file '{correct_file_path}' is not valid JSON.")
        return data
